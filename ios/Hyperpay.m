  ...
  for (NSString *key in options) {
    ...
    if (error) {
      ...
      reject(@"transactionPayment",error.localizedDescription, error);
       ...
       params.shopperResultURL = @"com.isalamah.clientapp.payments://result";
...
//          resolve(transactionResult);
          [self sendEventWithName:@"transactionStatus" body:@{@"redirectUrl": transaction.redirectURL.absoluteString, @"status":@"pending", @"checkoutId": transaction.paymentParams.checkoutID}];
          
        }  else if (transaction.type == OPPTransactionTypeSynchronous) {
          NSLog(@"%s", "sync");
          
          [self sendEventWithName:@"transactionStatus" body:@{ @"status":@"complete", @"checkoutId": transaction.paymentParams.checkoutID}];
        } else {
